...
# Загружаем данные
df = pd.read_csv('./train_data.csv', parse_dates=['date'], sep=',')
...
grouped = df_filtered.groupby(['cluster_id', 'year', 'week']).agg(
    ...
    total_bites=('amount_of_bites', 'sum'),
).reset_index()
...
grouped['week_number'] = grouped['week']


# Убираем временные колонки, оставляем нужные
result = grouped[['cluster_id', 'year', 'week_number', 'week_date_range', 'calendar_week',
                  'elevation', 'avg_snow_depth', 'season_start_date', 'total_bites']]

result['mean_bites_for_year'] = result.groupby(['cluster_id', 'year'])['total_bites'].transform('mean').round(0).astype(int)
# Сохраняем результат
result.to_csv('weekly_train_data.csv', index=False)

# Показываем первые строки
print(result.head(10))

# Информация о количестве недель по кластерам
print("\nКоличество недель по кластерам (только с >m днями):")
...
print(f"Осталось недель (>=5 дней): {total_weeks_after}")
